...
On both Mac and Windows, runtime `DATABASE_URL`, `REDIS_URL`, all Supabase settings, the Storage bucket, and the Gemini configuration must identify the same shared services. A process-only `MIGRATION_DATABASE_URL` must identify the same PostgreSQL database while using the separate migration owner. Keep it blank or absent in shared application environment files. `NEXT_PUBLIC_API_URL` and `WEB_ORIGIN` remain localhost because each browser calls the API running on its own computer. Set `NEXT_PUBLIC_ALLOW_LOCAL_API=true`; Next.js uses production mode during a build even when that build will run only on the local computer.

The current verified Gemini default is stable `gemini-3.6-flash` on the Gemini Developer API `v1beta` endpoint. API and worker must use the exact same `GEMINI_MODEL`: the worker records the model used and the API/store intentionally rejects a result produced under a different configured model. The adapter uses a 45-second request ceiling and one SDK transport attempt; BullMQ owns the outer retry policy. Changing provider, backend, model or API version requires a new bounded synthetic compatibility check before live medical-data use.
...
The effective value comes from the first source that defines a variable: an existing process variable, then root `.env.local`, then root `.env`. Remove stale `DATABASE_URL`, `MIGRATION_DATABASE_URL`, `REDIS_URL`, and local-allow overrides from `.env.local`; otherwise they take priority over the intended application values. Safe startup diagnostics show only local/remote classification and the Redis hostname, never credentials or full URLs.
...
The API, worker and `@prisma/adapter-pg` runtime consume the server-only `DATABASE_URL`. Prisma CLI prefers the separate process-only `MIGRATION_DATABASE_URL` and falls back to `DATABASE_URL` when the override is absent or blank. Both URLs must identify the exact same logical database. The runtime URL uses the least-privilege `medvault_runtime` identity; the migration URL uses the retained migration owner. Keep the owner URL out of root `.env`, `.env.local`, and application deployment environments; inject it only into a dedicated migration process/job and remove it immediately afterward. An arbitrary `DIRECT_URL` still has no effect.
...
Use the dedicated least-privilege runtime PostgreSQL role rather than a provider owner/admin role. Migration ownership and runtime access are separate. The exact application-object migration passed the isolated EX-02 rehearsal but remains unapplied to hosted PostgreSQL pending durable recovery-key custody and a fresh live inventory.
...
- API: `NODE_ENV=production`, hosted least-privilege `DATABASE_URL`, no migration credential, both local-service allow flags false, a TLS-enabled hosted `REDIS_URL` using `rediss://`, `SUPABASE_URL`, `SUPABASE_ANON_KEY`, `SUPABASE_SERVICE_ROLE_KEY`, `SUPABASE_STORAGE_BUCKET`, HTTPS `WEB_ORIGIN`, Redis-backed/fail-closed rate limiting, and the documented API/reconciliation settings.
- Worker: `NODE_ENV=production`, the exact same least-privilege `DATABASE_URL` and TLS-enabled `REDIS_URL` as the API, no migration credential, both local-service allow flags false, the matching `SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`, bucket, `GEMINI_API_KEY`, model, concurrency, and monitoring settings.
- Migration operator/job: an explicitly injected, process-only hosted `MIGRATION_DATABASE_URL` for the same target using the migration owner, `MEDVAULT_ALLOW_LOCAL_DATABASE=false`, guaranteed credential cleanup after the command, and no browser/application exposure.
...
| `DATABASE_URL`                      | API, worker; Prisma fallback | Secret/server             | Required                                        | Exact least-privilege hosted runtime connection                  | `postgresql://runtime.project:password@pooler.example.test:5432/postgres?sslmode=require` |
| `MIGRATION_DATABASE_URL`            | Prisma CLI only              | **Secret/migration job**  | Process-only after runtime hardening            | Same database through the separate migration owner; never shared | Leave blank in application `.env`; inject for the command only                            |
...
| `GEMINI_MODEL`                      | API, worker                  | Server                    | Optional                                        | Shared Gemini model identifier and persisted-model guard         | `gemini-3.6-flash`                                                                        |
...
- Never put `SUPABASE_SERVICE_ROLE_KEY`, `GEMINI_API_KEY`, runtime/migration database credentials, or Redis credentials in a `NEXT_PUBLIC_*` variable.
...
- Keep runtime database, migration-owner, Redis, service-role and Gemini credentials out of every `NEXT_PUBLIC_*` variable.
...
1. Stop local PostgreSQL and Redis and configure the exact hosted least-privilege runtime and queue URLs in the application environment. Keep `MIGRATION_DATABASE_URL` blank or absent there.
2. In a dedicated migration process/job, inject the exact migration-owner `MIGRATION_DATABASE_URL`, run `pnpm db:generate` and `pnpm db:deploy`, remove the owner credential even on failure, and end that process/job.
3. From a clean application process that has no migration credential, run `pnpm dev`.
4. Confirm API and worker diagnostics both say `database=remote` and `redis=remote` with the expected safe hostname.
5. Save a synthetic profile, refresh, sign out, clear browser application data, sign in, and confirm the same profile returns.
6. Upload a synthetic report; confirm its database row, private object, BullMQ job, worker processing, extraction, and `NEEDS_REVIEW` state.
7. Sign in to the same account from the other computer with the same hosted settings and confirm the same profile, document, signed preview, and extraction.
